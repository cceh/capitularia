...
<!--	<!-\- use values: 0/100/500/1000 -\->
	<xsl:param name="pSplit">500</xsl:param>-->
...
<!-- BAUSTELLEN/ToDO:
	* ACHTUNG: Bei zu kleinen Chunks kommt es vor, dass im alphabetischen Index bestimmte Buchstaben fehlen/wegfallen! => XPath-Ausdruck entsprechend anpassen! (Nur...wie?!)
	* xsl:key einbauen?!
	* "//item" um-/ausformulieren
	* ...
	-->
...
<!--		<xsl:text> </xsl:text>-->
	...
	<xsl:template match="tei:analytic/tei:title[@type='sub']">
		...
		<xsl:if test="following-sibling::tei:title[@type='sub']">
			<!--<xsl:text>{T2}</xsl:text> <!-\- testweise -\->-->
			<xsl:text>. </xsl:text>
		</xsl:if>
	</xsl:template>
...
<!--	<xsl:template match="tei:biblStruct[@type='bookSection']/tei:monogr/tei:editor[.='Unbekannt']">
	...
	<xsl:template match="tei:biblStruct[@type='bookSection']/tei:monogr/tei:editor[.!='Unbekannt']">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:text>{T3}</xsl:text> <!-\- testweise -\->
				<xsl:text>, </xsl:text>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>-->
		...
		<tr>
			<td class="Lit">
				...
				<xsl:if test="tei:monogr/tei:imprint/tei:biblScope[@unit='volume']">
					...
					<!-- Untertitel -->
				<xsl:if test="tei:monogr/tei:title[@type='sub']">
					<xsl:choose>
						<xsl:when test="tei:monogr/tei:imprint/tei:biblScope[@unit='volume']">
							<xsl:text>. </xsl:text>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="$vPassendesTrennzeichen"/>
		...
		<tr>
			<td class="Lit">
				...
				<xsl:variable name="vPassendesTrennzeichen">
					...
					<!-- Bandnummer -->
				<xsl:if test="tei:analytic/tei:imprint/tei:biblScope[@unit='volume']">
					<xsl:value-of select="$vPassendesTrennzeichen" />
					<xsl:apply-templates select="tei:analytic/tei:imprint/tei:biblScope[@unit='volume']" />
				</xsl:if>					
					<!-- Untertitel -->
				<xsl:if test="tei:analytic/tei:title[@type='sub']">
					<xsl:choose>
						<xsl:when test="tei:analytic/tei:imprint/tei:biblScope[@unit='volume']">
							<xsl:text>. </xsl:text>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="$vPassendesTrennzeichen" />							
				...
				<xsl:text> </xsl:text>
					<!-- Erscheinungsdatum -->
				<xsl:text>(</xsl:text><xsl:apply-templates select="tei:monogr/tei:imprint/tei:date" /><xsl:text>)</xsl:text>
					<!-- Seitenangabe -->
				<xsl:text> S. </xsl:text>
