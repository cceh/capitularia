...
class Witness
{
    ...
    public function extract_corresp ($corresp, &$errors)
    {
     ...
     * @param string $corresp The corresp
     *
     * @return int The number of corresps.
     */

    public function count_corresps ($corresp)
    {
        $n = 0;
        foreach ($this->get_nodes_for ($corresp) as $node) {
            if ($node->getAttribute ('prev')) {
