...
def output_ping(http, baseurl, extended):
    ...
    for k, c in cores["status"].items():
        ...
        outputE += "\n{} - Status: {} | {}_size={}B {}_numdocs={} {}_uptime={}s".format(
            k, status["status"], k, size, k, numdocs, k, uptime
        )

    if state == 2:
        print("CRITICAL: couldn't ping {} cores".format(",".join(nokCores)))
    else:
        print("SOLR OK: {} ping correctly".format(",".join(okCores)))
...
def output_replication_master(http, baseurl, extended):
    ...
    for k, c in cores["status"].items():
        ...
        outputE += "\n{} - isMaster: {} -- replicationEnabled: {}".format(
            k, master, enabled
        )

    if state == 2:
        print(
            "CRITICAL: {} are not masters/replication disabled".format(
                ",".join(nokCores)
            )
        )
    else:
        print("SOLR OK: {} are all master & replicating!".format(",".join(okCores)))
...
def output_replication_slave(http, baseurl, extended, warn, critical):
    ...
    for k, c in cores["status"].items():
        ...
        masterIndex = repl["details"]["slave"]["masterDetails"]["master"][
            "replicableVersion"
        ]
        masterGen = repl["details"]["slave"]["masterDetails"]["master"][
            "replicableGeneration"
        ]

        enabled = repl["details"]["slave"]["masterDetails"]["master"][
            "replicationEnabled"
        ]
    ...
    if state == 2:
        print(
            "CRITICAL: {} are not slaves/replication disabled".format(
                ",".join(nokCores)
            )
        )
    elif state == 1:
        ...
        print("SOLR OK: {} are all slaves & replicating!".format(",".join(okCores)))
...
def output_compare(http, baseurl, baseurl2, extended):
    ...
    if len(intersect) > 0:
        print(
            "CRITICAL: {} core(s) is/are not on both solr servers | count={}".format(
                ",".join(intersect), len(intersect)
            )
        )
        state = 2
    else:
        print(
            "SOLR OK: all cores exist on both servers | count={}".format(len(intersect))
        )
...
def main():
    ...
    parser.add_argument(
        "-H", "--host", help="Hostname of the solr server", required=True
    )
    parser.add_argument(
        "-p", "--port", help="Port of the solr server", default=8983, type=int
    )
    parser.add_argument(
        "-W", "--path", help="Path to the solr instance", default="solr"
    )
    parser.add_argument("-t", "--timeout", help="timeout", default=15, type=int)
    parser.add_argument(
        "--extended", help="Add extended info where possible", action="store_true"
    )
    parser.add_argument("-P", "--ping", help="Ping the solr cores", action="store_true")
    parser.add_argument(
        "-r", "--replication", help="Ping the solr cores", default="slave"
    )
    parser.add_argument(
        "-w", "--warning", help="Warning threshold for replication", default=1, type=int
    )
    parser.add_argument(
        "-c",
        "--critical",
        help="Critical threshold for replication",
        default=3,
        type=int,
    )
    parser.add_argument(
        "-C", "--compare", help="Compare cores on 2 solr servers", action="store_true"
    )
    parser.add_argument("--host2", help="Host to compare to")
    parser.add_argument(
        "--port2", help="Port of host to compare to", default=8983, type=int
    )
    ...
    try:
        if args.ping:
            ...
            output_replication_slave(
                http, baseurl, args.extended, args.warning, args.critical
            )
        elif args.compare:
