...
def deps (path):
    ...
    for e in tree.xpath ('//comment ()[1]'):
        m = re.search (r'^\s*Input files?:\s+(.*)$', e.text, re.IGNORECASE | re.MULTILINE)
        if m:
            for fn in m.group (1).split ():
                dep = URIRef (os.path.join (directory, fn))
                g.add ( (path, inputs, dep) )
                add_ext (dep)
        m = re.search (r'^\s*Output files?:\s+(.*)$', e.text, re.IGNORECASE | re.MULTILINE)
        if m:
            for fn in m.group (1).split ():
                dep = URIRef (os.path.join (directory, fn))
                g.add ( (path, outputs, dep) )
                add_ext (dep)
        m = re.search (r'^\s*Output URLs?:\s+(.*)$', e.text, re.IGNORECASE | re.MULTILINE)
        if m:
            for fn in m.group (1).split ():
                dep = URIRef (os.path.join (directory, fn))
                g.add ( (path, urls, dep) )
...
def render_dot ():
    with open (ARGS.output, 'w') as fp:
        ...
        # for s, p, o in g.triples ( (None, isa, lit_xsl) ):
        #     fp.write ('  "%s" [color=blue]\n' % s)
        # for s, p, o in g.triples ( (None, isa, lit_xml) ):
        #     fp.write ('  "%s" [color=green]\n' % s)
        # for s, p, o in g.triples ( (None, isa, lit_html) ):
        #     fp.write ('  "%s" [color=red]\n' % s)
