...
const REGEX_ALLOWED = '/^\$dummy_|_dummy$/';
 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class phpcs_Sniffs_CodeAnalysis_UnusedFunctionParameterSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if ($foundContent === true && count($params) > 0) {
            foreach ($params as $paramName => $position) {
                if (preg_match(REGEX_ALLOWED, $paramName) === 0) {
