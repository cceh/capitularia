...
class FunctionDeclarationSniff implements Sniff
{
     ...
     * @var integer
     */
    public $requiredSpacesBeforeOpen = 1; // Highlander
    ...
    {
        $this->requiredSpacesBeforeOpen  = (int) $this->requiredSpacesBeforeOpen; // Highlander
        ...
        if ($tokens[$stackPtr]['code'] === T_FUNCTION
            ...
            if ($spaces !== $this->requiredSpacesBeforeOpen) { // Highlander
            ...
            // Highlander
            if ($spaces !== $this->requiredSpacesBeforeOpen) {
                $error = 'Expected {$this->requiredSpacesBeforeOpen} space(s) before opening parenthesis; %s found';
                $data  = [$spaces];
                $fix   = $phpcsFile->addFixableError($error, $openBracket, 'NoSpaceBeforeOpenParen', $data);
                if ($fix === true) {
                    $phpcsFile->fixer->beginChangeset();
                    $phpcsFile->fixer->insertToken($openBracket - 1, ' ');
